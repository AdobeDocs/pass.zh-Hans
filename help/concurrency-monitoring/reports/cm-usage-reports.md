...
**并发监控使用情况报表**&#x200B;服务可通过REST API使用，该API可将insight提供给客户应用程序报告的并发使用情况。

## 先决条件 {#usage-rep-prerequisites}

要访问并发监控使用报告产品，客户必须首先联系并发监控[支持团队](mailto:tve-support@adobe.com)，他们将执行必要的步骤以允许您访问API产品。 有关[CMU API访问](/help/concurrency-monitoring/reports/cmu-api-access.md)的详细信息。
...
为获得最佳数据准确性，我们建议在此页面上显示的报告[CMU报告示例](/help/concurrency-monitoring/reports/cm-usage-reports-examples.md)
