...
CMU API提供了基础OLAP多维数据集的分层视图。 维度层次结构中的每个资源（[维度](/help/concurrency-monitoring/reports/cm-usage-reports.md#dimensions-2-filter-metrics)，映射为URL路径区段）生成包含（聚合）当前选择的[个量度](/help/concurrency-monitoring/reports/cm-usage-reports.md#monitor-metrics)的报表。 每个资源都指向其父资源（用于累计）及其子资源（用于向下钻取）。 切片和切片是通过将维度固定到特定值或范围的查询字符串参数实现的。
...
`GET` API终结点的`https://mgmt.auth.adobe.com/cmu/v2`将返回包含以下内容的表示形式：
...
目前唯一可用的HTTP方法是GET。 未来版本中可能会提供对OPTIONS/HEAD方法的支持。
