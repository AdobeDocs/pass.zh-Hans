...
**A：**&#x200B;并发监视无法直接区分实时内容与点播视频(VOD)。 视频播放器必须知道正在播放的内容类型，并在[会话初始化调用](/help/concurrency-monitoring/api/cm-api-overview.md#session-initial)期间发送此信息（并发监视所需）。 常规工作流如下所示：
...
* [并发监控标准元数据属性](/help/concurrency-monitoring/technical/standard-metadata-attributes.md)
* [并发监控API概述](/help/concurrency-monitoring/api/cm-api-overview.md)
