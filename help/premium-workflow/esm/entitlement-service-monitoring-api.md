...
>
> * 按照[检索客户端凭据](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-client-credentials.md) API文档中的说明获取客户端凭据。
> * 按照[检索访问令牌](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/apis/dynamic-client-registration-apis-retrieve-access-token.md) API文档中的说明获取访问令牌。
>
> 有关如何创建已注册的应用程序和下载软件语句的详细信息，请参阅[动态客户端注册概述](/help/authentication/integration-guide-programmers/rest-apis/rest-api-dcr/dynamic-client-registration-overview.md)文档。
...
端点URL根路径将返回单个记录中的总体汇总量度，以及指向可用深入分析选项的链接。 API版本被映射为端点URI路径的尾随区段。 例如，`https://mgmt.auth.adobe.com/esm/v3`表示客户端将访问WOLAP版本3。
...
以下深入分析树说明ESM 3.0中适用于[程序员](#progr-dimensions)和[MVPD](#mvpd-dimensions)的维度（资源）。


### 可用于程序员的维度 {#progr-dimensions}
...
| 参数 | 可选 | 描述 | 默认值 | 示例 |
| --- | ---- |-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| ---- | --- |
| access_token | 是 | DCR令牌可作为标准授权持有者令牌进行传递。 | 无 | access_token=XXXXXX |
...
| 结束 | 是 | 报告的结束时间（以毫秒为单位） | 服务器的当前时间 | end=2024-07-30 |
...
1. 在URL路径的最后一个段后附加一个“文件扩展名”：例如`/esm/v3/media-company/year/month/day.xml`。 如果URL包含查询字符串，则扩展名必须位于问号之前： `/esm/v3/media-company/year/month/day.csv?mvpd= SomeMVPD`
...
示例： https://mgmt.auth.adobe.com/esm/v3/year/month.csv将生成一个名为`report__2024-07-20_2024-08-20_1000.csv`的文件，该文件包含以下内容：
