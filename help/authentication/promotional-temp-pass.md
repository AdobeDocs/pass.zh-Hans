...
* **用户标识符信息，例如电子邮件地址传播** （发送验证和授权流中的用户电子邮件地址）。 Adobe Pass身份验证需要电子邮件来绑定身份验证和授权令牌(与 `device_ID`，在所有调用中必需)。
...
>Adobe Pass身份验证没有内置机制可在X分钟后停止免费流式传输。 Adobe Pass身份验证将停止发布 **授权** 和 **短媒体** 用户使用Y空闲资源后的令牌。 一旦促销临时密码过期，将由程序员限制访问。
