...
| 不适用。 | 功能 | 描述 | 优先级 | 注释 |
|------|---------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|----------------------------------------------------------------------------------------------------------------------------------------------------------|
| 1.1 | [程序员启动的登录](/help/authentication/integration-guide-mvpds/authn-oauth2-protocol.md) | 查看器选择MVPD并从程序员品牌的网站或应用程序启动身份验证(AuthN)流程。 | A+ |                                                                                                                                                          |
| 1.2 | [基于渠道的授权](/help/authentication/integration-guide-mvpds/authz-usecase.md) | 经过身份验证后，授权(AuthZ)可以在后台发生，只需将网络渠道标识符和用户标识符传递到MVPD。 | A+ |                                                                                                                                                          |
| 1.3 | 用户ID持久性 | MVPD提供了一个经过模糊处理的永久用户ID。 | A |                                                                                                                                                          |
| 1.4 | [单点登录支持](/help/authentication/integration-guide-programmers/legacy/sso-access/sso-support.md) | 查看器在一个品牌的网站上使用MVPD登录，然后可以转到另一个品牌，并且不会提示您再次登录。 身份验证会话在品牌之间共享。 SSO支持同时适用于网站和移动/设备应用程序。  对于MVPD，需要返回用户ID或可用于跨品牌进行身份验证的其他用户令牌。 | A |                                                                                                                                                          |
| 1.5 | 设备优化的登录用户体验(UX) | MVPD支持调整登录屏幕的大小，以便它适合视图正在使用的设备的尺寸。 | A |                                                                                                                                                          |
| 1.6 | 默认MVPD选取器徽标 | MVPD提供了一个指向相应尺寸（112x33像素）的默认徽标的URL。 | A | 徽标应由MVPD托管，并由CDN缓存。 |
| 1.7 | [服务提供商范围](/help/authentication/integration-guide-mvpds/serv-provider-scoping.md) | MVPD支持在AuthN请求中传递品牌标识符（请求者值）。 | A- | 这将启用特定于服务提供商的登录体验。 |
| 1.8 | [多渠道授权](/help/authentication/integration-guide-mvpds/mvpd-preflight-authz.md#preflight-multich-authz) | MVPD支持程序员在单个授权请求中发送多个渠道。 | B+ |                                                                                                                                                          |
| 1.9 | 基于iFrame或“JS弹出窗口”的身份验证 | 该程序员能够将登录流集成到iFrame或弹出窗口体验中，而不是HTTP重定向中。 | B |                                                                                                                                                          |
| 1.10 | [程序员启动的注销](/help/authentication/integration-guide-mvpds/usecase-mvpd-logout.md) | 查看者在程序员的网站或应用程序上和MVPD中都有一个经过身份验证的会话。 查看者可以从程序员站点启动联合注销，注销操作还会清除MVPD门户上的会话。 | B | 从程序员的角度来看，确保共享计算机受到更好的保护，免遭滥用。 |
| 1.11 | [自定义授权错误消息](/help/authentication/integration-guide-programmers/legacy/error-reporting/error-reporting.md) | MVPD传递其自身的自定义错误字符串，该字符串适用于向用户显示的程序员的联合站点或应用程序。 | B | 支持追加销售方案 |
| 1.12 | 身份验证响应中的&#x200B;**用户元数据** | MVPD AuthN响应可以包含用户元数据，这些元数据在权利流期间充当对用户体验进行个性化的提示。 此要求允许从MVPD向程序员发送家长控制提示。 | B- |                                                                                                                                                          |
| 1.13 | MVPD启动的身份验证 | 查看器在MVPD门户上成功完成身份验证会话，然后导航到程序员的TVE站点。 系统不会提示用户输入MVPD选取器，并且会自动对其进行身份验证。 | B- |                                                                                                                                                          |
