...
* **身份验证流程中生成的事件**（使用MVPD的实际登录）
   ...
   * AuthN挂起的通知 — 如果用户使用其MVPD成功登录，则在用户被重定向回Adobe Pass身份验证时生成此信息。
...
调用时自动触发 `getAuthorization()`，并且只涉及使用Adobe Pass身份验证进行的检查。 MVPD不参与此流。
...
因此，MVPD会向Adobe Pass身份验证发送对身份验证请求的肯定响应，并且Adobe Pass身份验证会创建 *身份验证令牌*.
...
* Adobe Pass身份验证 *非* 跟踪视频播放是否在生成媒体令牌后实际开始。 例如，如果内容存在地域限制，即使流从未实际开始，该交易仍会计为成功的播放请求。
