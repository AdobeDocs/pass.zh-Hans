...
如果客户端实施已经存在，则它不能替代该实施，因为如果没有访客ID，则无法跟踪用户活动超出以下特定事件。 如果客户在Pass调用中提供访客ID，那么我们可以解锁另一种类型的Analytics集成（实时），该集成可以将所有Pass事件与现有客户数据联接，有关这种新型的可能集成的更多详细信息，请单击此处：“[在Adobe Pass身份验证中使用Experience Cloud ID](/help/authentication/integration-guide-programmers/features-standard/analytics/exp-cloud-id-authn.md)”
