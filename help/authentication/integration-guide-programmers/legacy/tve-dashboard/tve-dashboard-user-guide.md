...
* **自定义架构**
  ...
  包含自定义方案的列表。 有关详细信息，请参阅[iOS/tvOS应用程序注册](/help/authentication/integration-guide-programmers/legacy/sdks/ios-tvos-sdk/iostvos-application-registration.md)。
...
* **MVPD数据**
   * **MVPD Id** — 系统中使用的MVPD唯一ID。
   * **显示名称** - MVPD的商业名称，可能用在用户的选取器中。
   * **徽标Url** - MVPD的商业徽标统一资源定位器(URL)。
   * **徽标预览** - MVPD的商业徽标预览(通过从上述统一资源定位器(URL)下载)。
* **常规设置**
   * **用户元数据键**
      * 可用于特定MVPD的元数据键。
   * **客户端数据属性**
...
* **身份验证设置**
   * **选择终结点**
      * 此字段指示MVPD公开的身份验证端点。 端点可以因所使用的身份验证协议而异。
   * **AuthN常规设置**
...
* **授权设置**
   * **选择终结点**
      * 此字段指示MVPD公开的授权端点。 端点可能会因所使用的授权协议而异。 可用的授权协议为SOAP、REST（用于无客户端设备）、SAML、XACML和OAUTH。
   * **AuthZ常规设置**
