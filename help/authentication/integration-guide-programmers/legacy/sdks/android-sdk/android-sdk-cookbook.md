...
1. 调用[`logout()`](#$logout)以注销用户。\
   ...
   答：  按照与身份验证工作流相同的模式，AccessEnabler域向UI应用程序层发出请求（通过`navigateToUrl()`回调）以创建WebView控件，并指示该控件在后端服务器上加载注销端点的URL。

   b.  同样，UI必须监视WebView控件的活动并检测控件在经历多次重定向时加载应用程序的自定义URL（即： `http://adobepass.android.app/`）的时间。 发生此事件后，UI应用程序层会关闭WebView并完成注销过程。
