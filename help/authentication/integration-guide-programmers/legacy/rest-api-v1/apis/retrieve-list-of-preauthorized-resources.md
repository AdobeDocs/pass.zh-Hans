...
| _deviceType_ | 设备类型（例如，Roku、PC）。</br></br>如果正确设置了此参数，则ESM在使用无客户端量度时提供按设备类型](/help/authentication/integration-guide-programmers/features-premium/esm/entitlement-service-monitoring-overview.md#clientless_device_type)划分的[量度，以便可以执行不同类型的分析，例如，Roku、AppleTV和Xbox。</br></br>请参阅，在传递量度中使用无客户端设备类型参数的[好处&#x200B;](/help/authentication/integration-guide-programmers/legacy/notes-technical/benefits-of-using-the-clientless-devicetype-parameter-in-pass-metrics.md)</br></br>**注意**：`device_info`将替换此参数。 |
| _设备用户_ | 设备用户标识符。 |
