...
* **在身份验证流中生成的事件**(实际使用MVPD登录)
   ...
   * 身份验证尝试通知 — 此信息在用户发送到MVPD登录站点时生成。
   * 身份验证通知挂起 — 如果用户成功使用其MVPD登录，则这会在用户        已重定向回Adobe Pass身份验证。
...
在这种情况下，重复数据消除应该不会产生任何影响，因为不同的MVPD用户应该不会收到相同的用户ID。 当对同一个MVPD中的两个不同品牌进行合计时，重复数据消除的效果应该会大得多。
...
此调用在调用`getAuthorization()`时自动触发，仅涉及使用Adobe Pass身份验证的检查。 此流程不涉及MVPD。
...
* MVPD端第一个接近此事件的是页面加载
* Adobe Pass身份验证不计算用户再次尝试登录MVPD页面的次数（密码不正确，请重试）
* 多次尝试均计为一次尝试
...
用户是MVPD的已知订阅者，通常使用付费电视订阅，但有时只能访问Internet。 发生成功的身份验证可能是因为用户使用其MVPD明确输入了有效凭据，或者是因为他们之前输入了有效凭据并选中了“记住我”（且上一个会话未过期）。

因此，MVPD向Adobe Pass身份验证发送对身份验证请求的肯定响应，并且Adobe Pass身份验证创建&#x200B;*AuthN令牌*。
...
如果授权被拒绝，则经验证的用户没有对所请求的编程的确认订阅。 最可能的原因是渠道不是用户订阅包的一部分，但这也可能反映用户只能从MVPD访问Internet。

对于某些MVPD，即使用户仅从MVPD订购Internet（无付费电视订购），仍可成功进行身份验证。 在这种情况下，即使用户请求授权的渠道位于基础包中，授权也会被拒绝。
...
* 由于AuthN和AuthZ令牌会缓存MVPD响应一段时间，因此播放请求事件成功是量度中最频繁的事件。
...
该值可以跨设备持久(MVPD为给定用户生成相同的值，无论登录发生在何处)或临时(为每个登录生成一个新值，MVPD将映射到该值。 通常，MVPD提供给Adobe Pass身份验证的值跨会话和设备进行持久性，但如前所述，持久性既不得到保证，也不得到验证。
